...
SKILL_SCRIPTS = Path("/Users/aaronnguyen/.codex/skills/etsy-10-image-maker/scripts")
...
def test_contract_example_passes_guard():
    assert guard.validate_contract(load_contract(), example_mode=True) == []


def test_cli_passes_contract_example():
...
def test_rejects_website_framing_marker():
...
def test_rejects_blank_mockup_only_marker():
    contract = load_contract()
...
def test_rejects_wrong_channel_and_non_ten_slots():
    contract = load_contract()
...
def test_rejects_missing_source_asset_and_buyer_question():
    contract = load_contract()
...
def test_rejects_product_evidence_below_channel_floor():
    contract = load_contract()
...
def test_rejects_missing_slot_ten_thank_you_and_usage_hint():
    contract = load_contract()
...
def test_guard_does_not_mutate_contract():
    contract = load_contract()
